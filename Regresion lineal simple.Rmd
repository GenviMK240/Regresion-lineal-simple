...
#Ajustar el modelo
modelo <- lm(num_cuentas ~ distancia, data = dist_ncuentas)
...
prediccionescuentas<- predict(modelo)

residuos <- data$num_cuentas - prediccionescuentas
...
En primer lugar los datos de las variables que van a formar parte del modelo, pero en cuetión de entrenamiento para ajustarlo. Para prepararlos, escogeria una serie de muestras (de lo que sea) que sean representativas de los datos, diferenicar las variables (independeintes y dependientes), corregir los valores faltantes o que den error además de transformar esas variables si fuera necesario, y, por último, hacer una division de esos datos en unos subconjuntos a los que se les aplicará el modelo a modo de prueba.

En segundo lugar mencionaría los conjuntos de datos de prueba

Este conjunto de datos se utiliza para evaluar el rendimiento del modelo ajustado en datos no vistos. Consiste en una muestra independiente de datos que no se ha utilizado para ajustar el modelo.
La preparación de este conjunto implica:
Aplicar las mismas transformaciones que se realizaron en el conjunto de datos de entrenamiento (si es necesario).
Asegurarse de que las variables independientes y la variable dependiente estén en el mismo formato que en el conjunto de datos de entrenamiento.
Verificar que no haya valores faltantes o datos atípicos en el conjunto de datos de prueba.
Asegurarse de que la distribución de las variables en el conjunto de datos de prueba sea similar a la del conjunto de datos de entrenamiento.
